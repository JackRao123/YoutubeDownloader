...
def downloadYoutube(videourl, path): #downloads a single youtube video
    ...
    if filesize < 99999999999999: #max_filesize: #this was used for something else, 
        ...
        try:   
            yt.download(path, filename = str(video_name) + ".mp4")
    ...
    while( ("quit" in command) ==False):
        if('youtube' in command):
            ...
            if (r'youtube.com/channel/' in command or '@' in command or 'youtube.com/c/' in command):
                ...
                download_channel(command, video_folder_path)
            else:
                #We are downloading a single video.
                downloadYoutube(command, video_folder_path)
        else:
